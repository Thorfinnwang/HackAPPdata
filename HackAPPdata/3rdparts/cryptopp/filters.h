...
public:
	...
	/// \since Crypto++ 5.6.0
	/// 翻译上面这段注释
	/// \brief 构造一个AuthenticatedDecryptionFilter
	/// \param c 一个AuthenticatedSymmetricCipher的引用
	/// \param attachment 一个可选的附加的转换
	/// \param flags 标志指示过滤器的行为
	/// \param truncatedDigestSize 摘要的大小
	/// \param padding \ref BlockPaddingSchemeDef "填充方案"
	/// \details 额外的认证数据应该在通道“AAD”中给出。
	/// \details <tt>truncatedDigestSize = -1</tt> 表示应该使用 \ref HashTransformation::DigestSize() "DigestSize"。
	AuthenticatedDecryptionFilter(AuthenticatedSymmetricCipher &c, 
	...
	//上面这个构造函数的参数解释
	//c:一个AuthenticatedSymmetricCipher的引用
	//attachment:一个可选的附加的转换
	//flags:标志指示过滤器的行为
	//truncatedDigestSize:摘要的大小
	//padding:填充方案
